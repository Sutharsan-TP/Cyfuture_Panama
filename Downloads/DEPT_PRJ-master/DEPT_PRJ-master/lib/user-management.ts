...
export async function updateStudentQuizStats(studentId: string) {
  ...
  // Update the student's avg_score and quizzes_taken
  const { error: updateError } = await supabase
    .from('students')
    .update({ avg_score: avgScore, quizzes_taken: quizzesTaken })
    .eq('id', studentId);
  if (updateError) {
    console.error('Error updating avg_score and quizzes_taken:', updateError);
  }
}
