...
export function generateStudentQuizDetailedPDF(
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(8);
  doc.text(`Generated by Dynamite Quiz • ${new Date().toLocaleDateString()} • Keep Learning & Growing!`, 15, footerY + 7);
...
export function generateAnalyticsReportPDF(analyticsData: AnalyticsData, facultyName: string): void {
  ...
  doc.setTextColor(255, 255, 255);
  doc.setFontSize(8);
  doc.text(`Dynamite Quiz Analytics Dashboard • Generated for ${facultyName} • ${new Date().toLocaleDateString()}`, 20, 292);
